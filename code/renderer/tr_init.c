...
void RB_TakeScreenshot( int x, int y, int width, int height, char *fileName ) {
	...
	byte *source = (byte*) Z_Malloc( p2width * p2height * 4 );
...
void R_LevelShot( void ) {
	...
	while (p2width<glConfig.vidWidth) p2width*=2;
	while (p2height<glConfig.vidHeight) p2height*=2;
	source = (byte*) Z_Malloc( p2width * p2height * 4 );
	#else
...
const void *RB_TakeVideoFrameCmd( const void *data )
{
	...
	int p2width=1, p2height=1;
	int xx, yy,aa;
	while (p2width<cmd->width) p2width*=2;
	while (p2height<cmd->height) p2height*=2;
	byte *source = (byte*) Z_Malloc( p2width * p2height * 4 );
	qglReadPixels( 0, 0, p2width, p2height, GL_RGBA, GL_UNSIGNED_BYTE, source );
	for (yy=0; yy<cmd->height; yy++)
		for (xx=0; xx<cmd->width; xx++)
			for (aa=0; aa<4; aa++)
				cmd->captureBuffer[yy*cmd->width*4+xx*4+aa]=source[yy*p2width*4+xx*4+aa];
	Z_Free(source);
	#else
